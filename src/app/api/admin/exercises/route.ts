...
export async function GET(req: NextRequest) {
  ...
  const exercises = await DB.query(
    `SELECT e.id, e.name, e.video_url, e.video_url_yt, e.description, e.muscles, e.joints,
            e.easier_id, easy.name as easier_name, e.harder_id, hard.name as harder_name
     FROM exercises e
...
export async function POST(req: NextRequest) {
  ...
  const result = await DB.run(
    `INSERT INTO exercises (name, video_url, video_url_yt, description, muscles, joints, easier_id, harder_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
