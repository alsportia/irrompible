...
export async function GET(req: NextRequest) {
  ...
  const programs = await DB.query(`
    SELECT p.programs_id as id, p.name, p.description, p.image_url,
           COUNT(s.sessions_id) as session_count
    FROM programs p
    LEFT JOIN sessions s ON s.programs_id = p.programs_id
    GROUP BY p.programs_id ORDER BY p.name ASC
  `);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existing = await DB.get('SELECT programs_id FROM programs WHERE name = ?', [name.trim()]);
    ...
    if (Array.isArray(sessions)) {
      for (const ses of sessions) {
        ...
        const sesResult = await DB.run('INSERT INTO sessions (session_code, name, programs_id) VALUES (?,?,?)', [sessionCode, ses.nombre_sesion || null, programId]);
