...
export async function importProgramFromExcel(
  ...
  for (const set of setsRows) {
    ...
    const setResult = await DB.run(
      ...
      [dbSessionId, set.description || null, set.block_label || null, set.block_type || 'normal', Number(set.num_sets) || 1, Number(set.block_order) || 1]
    );
    excelSetIdToDbId.set(String(set.set_id), setResult.id);
  }

  // Create set_exercises
  for (const se of setExRows) {
    ...
    await DB.run(
      ...
      [dbSetId, dbExId, Number(se.ex_order) || 1, se.repeticiones || null, se.tiempo || null]
    );
  }
